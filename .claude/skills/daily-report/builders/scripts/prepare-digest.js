...
const FEED_X_URL = 'https://raw.githubusercontent.com/zhangyanbo2007/follow-builders/main/feed-x.json';
const FEED_PODCASTS_URL = 'https://raw.githubusercontent.com/zhangyanbo2007/follow-builders/main/feed-podcasts.json';
const FEED_BLOGS_URL = 'https://raw.githubusercontent.com/zhangyanbo2007/follow-builders/main/feed-blogs.json';
const FEED_YOUTUBE_URL = 'https://raw.githubusercontent.com/zhangyanbo2007/follow-builders/main/feed-youtube.json';

const PROMPTS_BASE = 'https://raw.githubusercontent.com/zhangyanbo2007/follow-builders/main/prompts';
const PROMPT_FILES = [
...
async function main() {
  ...
  // Delivery config for LLM — strip sensitive credentials (appid, appsecret, token)
  const safeDelivery = {
    method: config.delivery?.method || 'stdout',
    ...(config.delivery?.chatId ? { chatId: config.delivery.chatId } : {}),
    ...(config.delivery?.email ? { email: config.delivery.email } : {}),
  };
  ...
  const output = {
    ...
    // Non-fatal errors — always include the field for consistency
    errors
  };
